package org.virutor.chess.uci;

import java.io.IOException;

import org.junit.Test;

public class UciServerTest {

	private static final String path = "c:\\devel\\C++\\EclipseCppWorkspace\\VirutorChessMake\\release\\windows32\\VirutorChess_1.1RC1\\bin\\VirutorChessUci_1.1.exe";
	 
	//TODO redo
	
	@Test
	public void test() throws IOException, InterruptedException {
		
		/*
		GameStateServer gameServer = GameStateServer.getInstance();
		gameServer.setStartPosition();
		
		Player white = new UciEngine(path, gameServer);
		Player black = new UciEngine(path, gameServer);
		//Player black = new HumanPlayer();
		
		gameServer.setPlayer(COLOR_WHITE, white);
		gameServer.setPlayer(COLOR_BLACK, black);

		gameServer.start();
				
		System.out.println("Starting");
		Thread.sleep(30000);
		
		System.out.println("Stopping");
		gameServer.stop();
		*/
		
		
		
	}
	
}
