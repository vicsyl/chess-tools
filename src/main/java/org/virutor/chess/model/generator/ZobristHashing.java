...
public class ZobristHashing {
	...
	private static final int[][] pieceTypes = new int[][] {
		{1, 0},
		{7, 6},
		{3, 2},
		{5, 4},
	...
	private static int getKindOfPiece(byte piece, byte color) {
		return pieceTypes[piece - 1][color];
	}
	
	private static int from120to64(int i) {
		return (i/10 - 2) * 8 + (i % 10) - 1;
	}
	
	private static long keyForPiece(Field field, int index) {
		return HASH_KEYS[64*getKindOfPiece(field.pieceType, field.color) + from120to64(index)];   
	}
	
	private static final long[] HASH_KEYS = new long[] {
	...
	public static void setPositionHash(Position position) {
	
		long hash = 0L; 
		
		// a) pieces
		for(int i = 0; i < 120; i++) {			
			hash = getNewHashForField(position, i, hash);			
		}
		
		
		//b castles
		// WARNING: this depends on Position constant real values !!		
		for(int i = 0; i < 4; i++) {
			if(position.castles[i]) {
				hash ^= HASH_KEYS[768+i];		
			}
		}
		
		//c en passant
		hash = updateHashForUnconditionalEp(position, hash);		
		
		//d turn
		if(position.colorToMove == Position.COLOR_WHITE) {
			hash ^= HASH_KEYS[780];
		}
	...
	private static long updateHashForUnconditionalEp(Position position, long hash) {
		
		if(position.possibleEpIndex != Position.OFF_BOARD) {
			// Conditional (unlike in fen and position.possibleEpIndex)
			int epField = position.colorToMove == Position.COLOR_WHITE ? position.possibleEpIndex-10 : position.possibleEpIndex+10;
			Field pawnDoublePushField = position.board[epField];			
			//there should be a pawn there 
			if(!checkForPiece(Piece.PIECE_PAWN, Position.oppositeColor(position.colorToMove), pawnDoublePushField)) {				
				//let's just be very tolerant here and ignore the ep field then
			} else if(checkForPiece(Piece.PIECE_PAWN, position.colorToMove, position.board[epField + 1]) || checkForPiece(Piece.PIECE_PAWN, position.colorToMove, position.board[epField - 1])) { 
				int epFile = (epField % 10) - 1;
				hash ^= HASH_KEYS[772 + epFile];
			}
		}
	...
	private static boolean checkForPiece(byte piece, byte color, Field onField) {
		return onField.color == color && onField.pieceType == piece;
	}
	
	
	private static final long getNewHashForField(Position position, int index, long inputHash) {
		
		if(position.board[index].color == Position.COLOR_BLACK || position.board[index].color == Position.COLOR_WHITE) {
			inputHash ^= keyForPiece(position.board[index], index);
		}
		return inputHash; 
	...
	/**
	 * Needs old position for "lost castles"
	 * @param oldPosition
	 * @param newPosition
	 * @param move
	 */
	public static void updatePositionHashAfterMove(Position oldPosition, Position newPosition, Move move) {
				
		//compute hash change for "lost castles"
		for(int i = 0; i < 4; i++) {
			if(oldPosition.castles[i] != newPosition.castles[i]) {
				newPosition.hash ^= HASH_KEYS[768+i];	
			}
		}
		
		//always account for switching the color to move
		newPosition.hash ^= HASH_KEYS[780];
		
		//possibly lost unconditional ep field from the old position
		newPosition.hash = updateHashForUnconditionalEp(oldPosition, newPosition.hash);	
		...
		switch(move.castle_ep_flag) {
			...
			case Move.CASTLE_EP_FLAG_BLACK_0_0_0:			
				int[] changeIndices = MoveGenerator.getCastleChangeIndices(move.castle_ep_flag);
				for(int i : changeIndices) {
					newPosition.hash = getNewHashForField(newPosition, i, newPosition.hash);			
				}			
				break;
		
			case Move.CASTLE_EP_FLAG_EP:
				
				//ep. - account for captured piece			
				int fileTo = move.to % 10;
				int rowFrom = move.from / 10;
				int capturedPawnIndex = rowFrom * 10 + fileTo;
				newPosition.hash = getNewHashForField(oldPosition, capturedPawnIndex, newPosition.hash);		
				
			case Move.CASTLE_EP_FLAG_NO_CASTLE:
				
				newPosition.hash = updateHashForUnconditionalEp(newPosition, newPosition.hash);	
				
				//from is now empty
				newPosition.hash = getNewHashForField(oldPosition, move.from, newPosition.hash);
				
				//to is now occupied with another piece - this accounts for promotions, too
				newPosition.hash = getNewHashForField(newPosition, move.to, newPosition.hash);

				//account for a possible piece capture; but only for non ep move!!!
				if(move.piece_captured != Piece.NO_PIECE && move.castle_ep_flag != Move.CASTLE_EP_FLAG_EP) {
					newPosition.hash = getNewHashForField(oldPosition, move.to, newPosition.hash);
				}			
